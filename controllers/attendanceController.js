...
exports.createAttendance = async (req, res) => {
  try {
    ...
    await db.query(
      'INSERT INTO attendance (user_id, check_in_time, check_in_latitude, check_in_longitude, check_in_image) VALUES (?, ?, ?, ?, ?)',
...
exports.updateAttendance = async (req, res) => {
  try {
    ...
    await db.query(
      'UPDATE attendance SET check_out_time = ?, check_out_latitude = ?, check_out_longitude = ?, check_out_image = ? WHERE id = ?',
...
async function getEmployeeOffDay(db, user_id, dateStr) {
  ...
  try {
    ...
    if (openYesterday) {
      ...
      if (effY) {
        ...
        if (isOvernightY && endDateStrY === todayStr) {
          const offToday = await getEmployeeOffDay(db, user_id, todayStr);
          const nextShift = offToday ? null : await getEffectiveShift(db, user_id, todayStr, defaultShiftId);
    ...
    if (offToday) {
      return res.status(400).json({
        message: 'Hari ini adalah jadwal OFF, tidak perlu absen',
        type: 'off_day',
        date: todayStr,
        reason: offToday.reason || null,
        source: offToday.source || null
      });
    }
    ...
    const offShiftDate = await getEmployeeOffDay(db, user_id, shiftDateStr);

    if (offShiftDate) {
      return res.status(400).json({
        message: 'Tanggal shift ini adalah jadwal OFF, tidak perlu absen',
        type: 'off_day',
        date: shiftDateStr,
        reason: offShiftDate.reason || null,
        source: offShiftDate.source || null
      });
    }
    ...
    const { start_time, tolerance_start_time } = effShift;
...
async function computeNilaiGradePayload(req, db) {
  ...
  const [[user]] = await db.query(
    'SELECT id, shift_id, location_id FROM users WHERE id = ? LIMIT 1',
    [userIdInt]
  );
  ...
  const [employeeOffRows] = await db.query(
    `SELECT DATE_FORMAT(off_date, '%Y-%m-%d') AS date
     FROM employee_off_days
     WHERE user_id = ?
  ...
  const employeeOffSet = new Set(employeeOffRows.map(r => r.date));
  ...
  for (let d = startDate; cmpDate(d, effectiveEnd) <= 0; d = addDays(d, 1)) {
    ...
    const isWorkdayBase = isWorkdayByLocation(dateObj, user.location_id);
    const isHoliday = holidaySet.has(d);
    const isEmployeeOff = employeeOffSet.has(d);
    const isWorkday = isWorkdayBase && !isHoliday && !isEmployeeOff;
    ...
    if (rec.leave_request_id) {
      ...
      if (
        lt === 'LATE_ARRIVAL' ||
        lt === 'LATE_ARRIVAL_OFFSITE' ||
        lt === 'EARLY_DEPARTURE' ||
        lt === 'EARLY_DEPARTURE_OFFSITE'
      ) {
        continue;
      }
...
exports.getNilaiGradeHtml = async (req, res) => {
  try {
    ...
    const totalSakit = num(data.total_sakit != null ? data.total_sakit : (data.sakit || 0) + (data.sd || 0));

    const pal = {
      A: { start: '#34d399', end: '#10b981', tint: '#86efac' },
      B: { start: '#60a5fa', end: '#3b82f6', tint: '#93c5fd' },
      C: { start: '#fbbf24', end: '#f59e0b', tint: '#fde68a' },
      D: { start: '#fb923c', end: '#f97316', tint: '#fdba74' },
      E: { start: '#f87171', end: '#ef4444', tint: '#fca5a5' }
    }[data.grade || 'E'];
